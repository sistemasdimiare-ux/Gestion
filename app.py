...
with st.container():
    with st.form(key=f"form_{st.session_state.form_iter}"):
        ...
        with col2:
            ...
            motivo_no_venta = st.selectbox("INDICAR MOTIVO DE NO VENTA (Solo para NO-VENTA)", ["", "CUENTA CON SERVICIO DE COMPETENCIA", "CLIENTE MOVISTAR", "MALA EXPERIENCIA", "CARGO FIJO INSUFICIENTE"])
        ...
        # Referidos siempre opcionales al final
        st.write("---")
        col_ref1, col_ref2 = st.columns(2)
        nom_referido = col_ref1.text_input("NOMBRE Y APELLIDO DE REFERIDO").upper()
        cont_referido = col_ref2.text_input("N° DE CONTACTO REFERIDO", max_chars=9)

        enviado = st.form_submit_button("🚀 Enviar Registro", use_container_width=True)

# --- 5. VALIDACIONES LÓGICAS (EL CORAZÓN DEL FORMULARIO) ---
if enviado:
    errores = []
    
    # Validaciones obligatorias para TODO registro
    ...
    if detalle == "SELECCIONA": errores.append("Debe elegir un DETALLE de gestión")

    # VALIDACIÓN SI ES NO-VENTA
    if detalle == "NO-VENTA":
        if not motivo_no_venta:
            errores.append("Para 'NO-VENTA', debe indicar el motivo")
    
    # VALIDACIÓN SI ES CUALQUIER OTRA COSA (VENTA, AGENDADO, ETC.)
    else:
        ...
        if venta_piloto is None: errores.append("Marcar si es Venta Piloto")

    if errores:
        st.error("❌ Por favor corrija:")
        for e in errores: st.markdown(f"* {e}")
    else:
        # Preparar datos para Dropbox
