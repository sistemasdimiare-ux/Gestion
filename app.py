...
with col2:
    ...
    motivo_no_venta = ""
    if detalle == "NO-VENTA":
        motivo_no_venta = st.selectbox("INDICAR MOTIVO DE NO VENTA *", ["", "CUENTA CON SERVICIO DE COMPETENCIA", "CLIENTE MOVISTAR", "MALA EXPERIENCIA", "CARGO FIJO INSUFICIENTE"], key="k_mot")
...
if enviado:
    ...
    if venta_piloto is None: errores.append("¿Venta Piloto?")
    if tipo_op == "SELECCIONA": errores.append("Tipo de Operación")
    if producto == "SELECCIONA": errores.append("Producto")
    if detalle == "SELECCIONA": errores.append("Detalle")
