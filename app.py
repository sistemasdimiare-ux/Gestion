...
def save_to_dropbox(df_nuevo):
    ...
    try:
        ...
        df_final = df_nuevo
    output = io.BytesIO()
    # Se requiere instalar xlsxwriter: pip install xlsxwriter
...
# Quitamos clear_on_submit para que no se borre si hay error de validación
with st.form("mi_formulario"):
    col1, col2 = st.columns(2)

    with col1:
        nombre_cliente = st.text_input("NOMBRE DE CLIENTE *", key="f_nom").upper()
        dni_cliente = st.text_input("N° DE DOCUMENTO (CLIENTE) *", max_chars=11, key="f_dni")
        email_cliente = st.text_input("EMAIL DE CLIENTE", key="f_mail").lower()
        tipo_op = st.selectbox("Tipo de Operación", ["SELECCIONA","CAPTACIÓN", "MIGRACIÓN", "COMPLETA TV", "COMPLETA MT", "COMPLETA BA"], key="f_tipo")
        producto = st.selectbox("PRODUCTO", ["SELECCIONA", "NAKED", "DUO INT + TV", "DUO TV", "DUO BA", "TRIO"], key="f_prod")
        cod_fe = st.text_input("Código FE", key="f_fe").upper()
        pedido = st.text_input("N° de Pedido *", max_chars=10, key="f_ped")

    with col2:
        detalle = st.selectbox("DETALLE", ["SELECCIONA", "VENTA FIJA", "NO-VENTA", "CLIENTE AGENDADO", "REFERIDO", "PRE-VENTA"], key="f_det")
        direccion = st.text_input("DIRECCION DE INSTALACION", key="f_dir").upper()
        contacto1 = st.text_input("N° DE CONTACTO 1 *", max_chars=9, key="f_cont1")
        contacto2 = st.text_input("N° DE CONTACTO 2", max_chars=9, key="f_cont2")
        venta_piloto = st.radio("¿Venta Piloto? *", ["SI", "NO"], index=None, horizontal=True, key="f_pilo")
        
        motivo_no_venta = st.selectbox("MOTIVO NO VENTA", ["", "COMPETENCIA", "CLIENTE MOVISTAR", "MALA EXPERIENCIA"], key="f_mot")
        nom_referido = st.text_input("NOMBRE REFERIDO", key="f_ref_n").upper()
        cont_referido = st.text_input("N° CONTACTO REFERIDO", max_chars=9, key="f_ref_c")
...
if enviado:
    ...
    if not contacto1 or len(contacto1) != 9 or not contacto1.isdigit(): errores.append("Contacto 1 (debe ser de 9 dígitos numéricos)")
    if venta_piloto is None: errores.append("Seleccionar si es Venta Piloto")
    ...
    if errores:
        ...
        try:
            ...
            # LIMPIEZA MANUAL SÓLO SI SE GUARDÓ
            for key in st.session_state.keys():
                if key.startswith("f_"):
                    if key in ["f_tipo", "f_prod", "f_det"]: st.session_state[key] = "SELECCIONA"
                    elif key == "f_pilo": st.session_state[key] = None
